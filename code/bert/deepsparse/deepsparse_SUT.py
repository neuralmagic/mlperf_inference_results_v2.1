...
def latencies_string(latencies_ns):
    NANO_SEC = 1e9
    MILLI_SEC = 1000

    latencies_ms = [i / 1e6 for i in latencies_ns]
    percentiles = [50.0, 80.0, 90.0, 95.0, 99.0, 99.9]
    buckets = np.percentile(latencies_ms, percentiles).tolist()
    buckets_str = ",".join(
        ["{}:{:.4f}".format(p, b) for p, b in zip(percentiles, buckets)]
    )

    total = np.sum(latencies_ms)

    rv = "qps={:.2f}, mean={:.4f}ms, sum={:.3f}ms, queries={}, percentiles={}".format(
        len(latencies_ms) / (total / MILLI_SEC),
...
class BERT_DeepSparse_SUT():
    ...
    def process_latencies(self, latencies_ns):
        print(latencies_string(latencies_ns))
