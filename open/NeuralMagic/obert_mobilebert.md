...
Then, we follow our gradual pruning setup to progressively prune the remaining 14 layers of MobileBERT model over the span of 30 training epochs. More specifically, we make use of the knowledge-distillation from the dense teacher, learning rate scheduler with rewinds and cubic sparsity scheduler with high initial pruning step. We prune the encoder part in the semi-structured 4-block pattern up to 50% sparsity.
